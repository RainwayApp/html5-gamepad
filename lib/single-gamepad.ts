import mappings from '../mappings'
import * as R from './typings'

type GamepadID = Gamepad['id']
type UserAgent = typeof navigator.userAgent

export default class SingleGamepad {
    mapping: R.Mapping
    constructor(public gamepad: Gamepad) {
        this.mapping = this.detectMapping(gamepad.id, navigator.userAgent)
    }

    detectMapping(id: GamepadID, browser: UserAgent) {
        for (var i = 0; i < mappings.length; i++) {
            if (isCompatible(mappings[i], id, browser)) {
                console.log('found mapping', mappings[i].name, 'for', id, 'on', browser)
                return clone(mappings[i])
            }
        }
        console.warn('no mapping found, using default for', id, 'on', browser)
        return clone(mappings[0])
    }

    axis(name: keyof R.Axes) {
        this.gamepad = navigator.getGamepads()[this.gamepad.index]! // HOTFIX
        var mapping = this.mapping.axes[name]
        if (!mapping) {
            return 0
        }

        if ('index' in mapping) {
            var index = mapping.index
            if ('from' in mapping && 'to' in mapping) {
                var x0 = mapping.from[0]
                var x1 = mapping.from[1]
                var y0 = mapping.to[0]
                var y1 = mapping.to[1]
                var x = this.gamepad.axes[index]
                if (x0 < x1 && y0 < y1) {
                    if (x < x0) {
                        return y0
                    } else if (x > x1) {
                        return y1
                    }
                }
                return y0 + (x - x0) * ((y1 - y0) / (x1 - x0))
            } else if ('negative' in mapping) {
                return -this.gamepad.axes[index]
            } else {
                return this.gamepad.axes[index]
            }
        }
        if (mapping.buttonAnalog !== undefined) {
            if (typeof this.gamepad.buttons[mapping.buttonAnalog] == 'number') {
                return this.gamepad.buttons[mapping.buttonAnalog]
            }
            if (
                this.gamepad.buttons[mapping.buttonAnalog] !== undefined &&
                this.gamepad.buttons[mapping.buttonAnalog].value !== undefined
            ) {
                return this.gamepad.buttons[mapping.buttonAnalog].value
            }
        }
        if (mapping.buttonPositive !== undefined && this.gamepad.buttons[mapping.buttonPositive].pressed) {
            return 1
        }
        if (mapping.buttonNegative !== undefined && this.gamepad.buttons[mapping.buttonNegative].pressed) {
            return -1
        }
        return 0
    }

    button(name: keyof R.Buttons) {
        this.gamepad = navigator.getGamepads()[this.gamepad.index]! // HOTFIX
        var mapping = this.mapping.buttons[name]
        if (!mapping) {
            return false
        }
        if ('index' in mapping && mapping.index !== undefined && this.gamepad.buttons[mapping.index] !== undefined) {
            return this.gamepad.buttons[mapping.index].pressed
        }
        if ('axis' in mapping && mapping.axis !== undefined) {
            if ('range' in mapping) {
                return (
                    this.gamepad.axes[mapping.axis] >= mapping.range[0] &&
                    this.gamepad.axes[mapping.axis] <= mapping.range[1]
                )
            }
            if (mapping.direction < 0) {
                return this.gamepad.axes[mapping.axis] < -0.75
            } else {
                return this.gamepad.axes[mapping.axis] > 0.75
            }
        }
        return false
    }
}

function isCompatible(mapping: R.Mapping, id: GamepadID, browser: UserAgent) {
    return mapping.supported.some(m => id.includes(m.id) && browser.includes(m.os))
}

function clone<T>(obj: T): T {
    return JSON.parse(JSON.stringify(obj))
}
